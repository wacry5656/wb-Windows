...
> 实现细节以 [`CURRENT_ARCHITECTURE.md`](CURRENT_ARCHITECTURE.md) 为准；本 README 与 `STRUCTURE.md` 侧重整体介绍，部分描述可能滞后于代码。

## ✨ 功能特性

- 📸 **错题上传**：上传错题图片或直接录入纯文字题目，记录标题和学科分类
- 📋 **错题列表**：查看所有错题，支持按学科、状态筛选和关键词搜索
- 📝 **详细笔记**：为每道错题添加解题思路、图片笔记
- 🤖 **AI 辅助**：基于通义千问（Qwen 兼容接口）生成知识点分析、思路指引、详细讲解与追问答疑
- ♻️ **复习模式**：基于掌握度和间隔重复的复习系统，支持多种排序方式
- ☁️ **多端同步**：可选的自建同步服务（`sync-server/`），在多设备间字段级合并错题
- 📊 **学习统计**：跟踪复习次数、掌握度与待复习数量
...
- ✅ AI 智能解答（通义千问 / Qwen 兼容接口，密钥仅保存在主进程）
- ✅ 本地持久化（Electron 主进程写入 `data/questions.json` 与 `data/images/`）
- ✅ 云同步（自建 `sync-server/`，字段级合并 + 删除墓碑）
- ✅ 学习统计与复习看板
